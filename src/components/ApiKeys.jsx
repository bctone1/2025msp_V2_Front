            ...
            <div className="flex justify-end gap-3">
              ...
              <button 
                ...
                className={`px-4 py-2 rounded-lg ${
                  !newKeyData.name || !newKeyData.key
          ...
          {apiKeys.map(apiKey => (
            <div key={apiKey.id} className="bg-white rounded-lg border p-6">
              ...
              <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg mb-6">
                <code className="flex-1 font-mono text-sm truncate">{apiKey.key}</code>
              ...
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  <div className="flex items-end gap-2">
                    <div className="text-2xl font-bold">{apiKey.usage.current.toLocaleString()}</div>
                    <div className="text-sm text-gray-500 mb-1">토큰</div>
                  </div>
                  {apiKey.usage.limit > 0 && (
                    <div className="mt-2">
                      <div className="flex justify-between text-xs text-gray-500 mb-1">
                        <span>사용량</span>
                        <span>{Math.round(apiKey.usage.current / apiKey.usage.limit * 100)}%</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div 
                          className="bg-blue-600 h-1.5 rounded-full" 
                          style={{ width: `${Math.min(100, Math.round(apiKey.usage.current / apiKey.usage.limit * 100))}%` }}
                        ></div>
                      </div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div className="flex items-end gap-2">
                    <div className="text-2xl font-bold">{apiKey.usage.limit > 0 ? apiKey.usage.limit.toLocaleString() : '무제한'}</div>
                    {apiKey.usage.limit > 0 && <div className="text-sm text-gray-500 mb-1">토큰</div>}
                  </div>
                </div>
